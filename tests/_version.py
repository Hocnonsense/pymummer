...
versionfile_source = "tests/genome/_version.py"
...
def versions_from_parentdir(parentdir_prefix, root: str, verbose):
...
def git_pieces_from_vcs(tag_prefix, root, verbose=False, runner=run_command):
...
def plus_or_dot(pieces: dict):
    """Return a + if we don't already have one, else return a ."""
    return "." if "+" in pieces.get("closest-tag", "") else "+"


def render_pep440(pieces: dict):
    ...
    if pieces["closest-tag"]:
        ...
        # exception #1
        rendered = "0+untagged." f"{pieces['distance']}.g{pieces['short']}"
        if pieces["dirty"]:
            rendered += ".dirty"
    return rendered
